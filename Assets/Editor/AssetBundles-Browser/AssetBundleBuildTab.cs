...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        internal void RealEnable()
        {
            ...
            m_EncryptMode = new ToggleData(
                false,
                "Encrypt Mode",
                "Encrypt Mode",
                m_UserData.m_OnToggles);

            m_ToggleData.Add(m_EncryptMode);
        ...
        private void ExecuteBuild()
        {
                ...
                if (m_ForceRebuild.state)
                {
                    ...
                    if (EditorUtility.DisplayDialog("File delete confirmation", message, "Yes", "No"))
                    {
                        try
                        {
                            ...
                            if (m_CopyToStreaming.state)
                            if (Directory.Exists(m_streamingPath + "/AssetBundles"))
                                Directory.Delete(m_streamingPath + "/AssetBundles", true);
                        }
            ...
            GameEditor.GameBundleTool.GenerageVersionList();
        ...
        public static void BuildPathMapping()
        {
            ...
            foreach (string bundleName in allBundleNames)
            {
                ...
                {
                    // AssetsPackage 下的路径 如Atlas/UITrade/res/activity_icon_change102_01.png
                    string packagePath = ABUtility.AssetsPathToPackagePath(assetPath);
                    // 图集资源不需要加
                    if (!regex.IsMatch(packagePath))
                    {
                        // 如：prefabs.assetbundle,Prefabs/Cube.prefab
            ...
            mappingList.Sort();
            // ab映射生成路径 ：Assets/AssetsPackage/AssetsMap.bytes
            string outputFilePath = ABUtility.PackagePathToAssetsPath(AssetBundleConfig.AssetsPathMapFileName);
